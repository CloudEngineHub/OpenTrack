...
class PlayG1TrackingGeneralEnv:
            ...
            if self._config.history_len > 0:
                obs["history_state"] = np.concatenate(
                    [
                        ...
                        np.concatenate([state.info["current_history"], state.info["last_motor_targets"]], axis=0)[None, :],
                    ],
                    axis=0,
                ).flatten()
    ...
    def get_obs(self, traj_data, info):
        ...
        avg_gyro_pelvis = gyro_pelvis * self._config.obs_scales_config.joint_vel
        avg_joint_vel = joint_vel[self.obs_joint_ids] * self._config.obs_scales_config.joint_vel
        world_state = np.concatenate(
            [
                ...
                avg_joint_vel,
                np.array([self.mj_data.qpos[2]], dtype=np.float32),
            ],
            axis=0,
        )
        ...
        return {
            ...
            "ref_world_state": ref_world_state,
        }, history
